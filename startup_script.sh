...
# Set DNAT with iptables. All TCP packets will end up reaching the same port, preventing us from having to open sockets on all ports.
iptables -t nat -p tcp -I PREROUTING -m multiport ! --dports 23,80,65535 -j DNAT --to-destination :4444
...
docker network create --subnet 172.21.0.0/16 no-outgoing
iptables --insert DOCKER-USER -s 172.21.0.0/16 -j REJECT --reject-with icmp-port-unreachable
iptables --insert DOCKER-USER -s 172.21.0.0/16 -m state --state RELATED,ESTABLISHED -j RETURN
/etc/init.d/iptables save
...
RUN deluser admin; \
    passwd --lock root; \
    echo -e 'support\nsupport' | adduser -h /tmp support; \
    echo -e 'service\nservice' | adduser -h /tmp service; \
    echo -e 'supervisor\nsupervisor' | adduser -h /tmp supervisor; \
    echo -e '12345\n12345' | adduser -h /tmp user; \
    echo -e '12345\n12345' | adduser -h /tmp guest; \
    echo -e '12345\n12345' | adduser -h /tmp admin; \
    echo -e '12345\n12345' | adduser -h /tmp system;
EOF
docker build -t localhoneypotimg .
docker run -it -d --restart always -h server --cap-drop=all --cap-add=setgid --cap-add=setuid --security-opt no-new-privileges --network no-outgoing -p 23:23 localhoneypotimg /bin/telnetd -F
docker run -it -d --restart always -h server --cap-drop=all --cap-add=setgid --cap-add=setuid --security-opt no-new-privileges --network no-outgoing -p 80:80 localhoneypotimg /bin/httpd -f
...
export NET_DEV="eth0"
export ROTATION_PERIOD_SEC="45" # 45 seconds -- FOR DEMO PURPOSES!
export SAVE_DIR="/tmp"
...
# Setup the TCP generic responder.
